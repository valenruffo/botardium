...
import { Activity, Users, MessageSquare, ShieldAlert, Settings, LogOut, BadgeCheck, ChevronDown, Loader2 } from "lucide-react";
...
export default function Dashboard() {
  ...
  const [currentView, setCurrentView] = useState<'dashboard' | 'crm' | 'campaigns' | 'message_studio' | 'guide' | 'admin_accounts'>('dashboard');
  ...
  // Data Fetching
  const leadsEndpoint = currentRoute === 'app'
    ? `http://localhost:8000/api/leads${selectedCampaignFilter ? `?campaign_id=${encodeURIComponent(selectedCampaignFilter)}` : ''}`
    : null;
  const { data: leads, mutate: mutateLeads } = useSWR<Lead[] | { leads?: Lead[] }>(leadsEndpoint, fetcher, { refreshInterval: 2000 });
  const { data: igAccountsData, mutate: mutateAccounts } = useSWR<IgAccount[]>(currentUserId ? `http://localhost:8000/api/accounts?user_id=${currentUserId}` : null, fetcher, { refreshInterval: 2000 });
  ...
  }>(currentRoute === 'app' ? 'http://localhost:8000/api/bot/status' : null, fetcher, { refreshInterval: 2000 });
  const { data: messageJobsData, mutate: mutateMessageJobs } = useSWR<{ jobs: MessageJob[] }>(currentRoute === 'app' ? 'http://localhost:8000/api/messages/jobs' : null, fetcher, { refreshInterval: 2000 });
  ...
  const accountWarmupAction = async (accountId: number, action: 'start' | 'cancel', durationMin = 10) => {
    try {
      const endpoint = action === 'start'
        ? `http://localhost:8000/api/accounts/${accountId}/warmup`
        : `http://localhost:8000/api/accounts/${accountId}/warmup-cancel`;
  ...
  const connectInstagramAccount = async (redirectToApp = false) => {
    ...
    try {
      const res = await fetch('http://localhost:8000/api/ig/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const reloginActiveAccount = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8000/api/accounts/${activeAccount.id}/relogin`, { method: 'POST' });
  ...
  const updateAccountType = async (accountId: number, accountType: 'mature' | 'new' | 'rehab') => {
    try {
      const res = await fetch(`http://localhost:8000/api/accounts/${accountId}/profile`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const completeAccountWarmupDay = async (accountId: number) => {
    try {
      const res = await fetch(`http://localhost:8000/api/accounts/${accountId}/account-warmup-day`, { method: 'POST' });
      const data = await res.json();
      if (!res.ok) {
  ...
  const addSource = (type: StrategyResult['sources'][number]['type']) => {
    setCampaignDraft((prev) => ({
      ...prev,
      sources: [...prev.sources, { type, target: '' }],
    }));
  };
  ...
  const removeSource = (index: number) => {
    setCampaignDraft((prev) => ({
      ...prev,
      sources: prev.sources.length === 1 ? prev.sources : prev.sources.filter((_, sourceIndex) => sourceIndex !== index),
    }));
  };

  const campaignAction = async (campaignId: string, action: 'start_warmup' | 'finish_warmup' | 'start_scraping' | 'pause' | 'delete') => {
    try {
      const res = await fetch(`http://localhost:8000/api/bot/${campaignId}/action`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const trustScore = 98; // Calculated by backend in real logic
  ...
  const bulkLeadAction = async (action: 'delete' | 'status', status?: string, all = false) => {
    try {
      ...
      const res = await fetch(`http://localhost:8000/api/leads/${endpoint}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const previewMessages = async (targetIds?: number[]) => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:8000/api/messages/preview', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ids,
  ...
  const queueMessages = async (targetIds?: number[], campaignId?: string | null) => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:8000/api/messages/queue', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ids,
  ...
  const saveLeadDraft = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/api/leads/${selectedLeadDraft.id}/draft`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const regenerateLeadDraft = async () => {
    if (!selectedLeadDraft?.id) return;
    try {
      const res = await fetch(`http://localhost:8000/api/leads/${selectedLeadDraft.id}/regenerate-draft`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          prompt_first_contact: messagePrompt,
  ...
  if (currentRoute === 'auth' || currentRoute === 'register') {
    const isLogin = currentRoute === 'auth';
    return (
      <main className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-md bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 shadow-2xl relative overflow-hidden animate-in fade-in zoom-in-95">
          <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-purple-500 to-indigo-500"></div>

          <div className="mb-8 text-center">
            <div className="w-16 h-16 mx-auto bg-gradient-to-br from-purple-500 to-indigo-600 rounded-2xl flex items-center justify-center font-bold text-white text-2xl shadow-lg shadow-purple-500/20 mb-6">
              B
            </div>
            <h1 className="text-2xl font-bold text-white mb-2">{isLogin ? 'Botardium Core' : 'Crear Cuenta Vitalicia'}</h1>
            <p className="text-slate-400 text-sm">{isLogin ? 'Inicia sesión en tu instancia SaaS' : 'Comienza a extraer leads de forma masiva'}</p>
          </div>
          ...
          <form onSubmit={async (e) => {
            ...
            try {
              ...
              const body = isLogin
                ...
                : { email: loginEmail, password: loginPassword, full_name: registerFullName };
              ...
              if (res.ok) {
                ...
                window.localStorage.setItem(AUTH_STORAGE_KEY, JSON.stringify({ userId: data.user_id, email: data.email }));
                toast.success(isLogin ? `Bienvenido, ${data.full_name || data.email}.` : "Cuenta creada exitosamente.");
                // Move to accounts view so they can connect IG or see logic
                setCurrentRoute('accounts');
              } else {
                toast.error(data.detail || "Error de autenticación");
              }
            } catch {
            ...
            {!isLogin && (
              <div>
                ...
                <input type="text" required value={registerFullName} onChange={(e) => setRegisterFullName(e.target.value)} placeholder="John Doe" className="w-full bg-slate-950/50 border border-slate-800 rounded-xl px-4 py-3 text-slate-100 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all placeholder:text-slate-600" />
              </div>
            )}

            <div>
              <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 block">Email / Licencia</label>
              <input type="text" required value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} placeholder="admin@moveup.com" className="w-full bg-slate-950/50 border border-slate-800 rounded-xl px-4 py-3 text-slate-100 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all placeholder:text-slate-600" />
            </div>

            <div>
              <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 block">Contraseña</label>
              <input type="password" required value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} placeholder="••••••••••••" className="w-full bg-slate-950/50 border border-slate-800 rounded-xl px-4 py-3 text-slate-100 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all placeholder:text-slate-600" />
            </div>

            <button type="submit" className="w-full bg-purple-600 hover:bg-purple-500 text-white font-bold py-3 px-4 rounded-xl transition-all shadow-lg hover:shadow-xl mt-6">
              {isLogin ? 'Ingresar al Portal' : 'Registrar Cuenta Segura'}
            </button>
          </form>

          <div className="mt-6 text-center">
            <button onClick={() => setCurrentRoute(isLogin ? 'register' : 'auth')} className="text-sm text-slate-400 hover:text-white transition-colors">
                  ...
                  <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1.4fr)_320px]">
                    ...
                    <div className="rounded-2xl border border-slate-800 bg-slate-950/50 p-4">
                      ...
                      <GlowSelect
                        ...
                        className="w-full"
                      />
                      <button onClick={() => setCurrentRoute('app')} className="w-full rounded-xl bg-white px-4 py-2 text-sm font-semibold text-slate-900 transition-all hover:bg-slate-200">
                        Usar esta cuenta
                      </button>
                      <button onClick={() => { setCurrentRoute('app'); setCurrentView('crm'); }} className="w-full rounded-xl bg-amber-500/15 px-4 py-2 text-sm font-medium text-amber-200 hover:bg-amber-500/25">
                        Ir al CRM para preparar envío
                      </button>
                      ...
                      <button
                        onClick={async () => {
                          if (confirm(`¿Estás seguro que deseas desconectar la cuenta @${acc.ig_username} del sistema?`)) {
                            try {
                              const res = await fetch(`http://localhost:8000/api/accounts/${acc.id}`, { method: 'DELETE' });
                              if (res.ok) {
                                mutateAccounts();
    ...
    <main className="min-h-screen bg-slate-950 text-slate-50 flex flex-col">
      ...
      <header className="border-b border-slate-800 bg-slate-900/50 p-4 flex items-center justify-between sticky top-0 z-50 backdrop-blur-md">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold tracking-tight">Botardium SaaS</h1>
        </div>
        <div className="flex items-center gap-4">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56 bg-slate-900 border-slate-800 text-slate-200">
              <DropdownMenuLabel>Mi Cuenta (Prime)</DropdownMenuLabel>
              <DropdownMenuSeparator className="bg-slate-800" />
              <DropdownMenuItem onClick={() => { setCurrentRoute('accounts'); setCurrentView('dashboard'); }} className="focus:bg-slate-800 focus:text-white cursor-pointer">
                <Settings className="mr-2 h-4 w-4" /> Configuración de Red / Filtros
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => { setCurrentRoute('accounts'); setCurrentView('dashboard'); }} className="focus:bg-slate-800 focus:text-white cursor-pointer">
              ...
              <DropdownMenuItem onClick={() => {
                ...
                toast.info("Sesión cerrada y datos protegidos.");
      ...
      <div className="flex flex-1">
        {/* Sidebar */}
        <aside className="w-64 border-r border-slate-800 bg-slate-900/30 hidden lg:block p-6">
          <div className="space-y-6">
            <div>
              ...
              <nav className="space-y-1">
                ...
                >
                  <Settings className="w-4 h-4" /> Cómo usarlo
                </button>
              </nav>
            </div>
        ...
        <div className="flex-1 p-6 md:p-10 max-w-7xl mx-auto w-full">
              ...
              <div className="w-full max-w-2xl rounded-3xl border border-cyan-500/20 bg-slate-900 p-6 shadow-2xl">
                ...
                <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-950/60 p-4">
                  ...
                  <div className="mt-4 flex flex-wrap gap-3">
                    <button onClick={regenerateLeadDraft} className="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-500">
                      Regenerar con IA
                    </button>
          ...
          {currentView === 'dashboard' && (
            <>
              <div className="mb-10 text-center sm:text-left space-y-2">
                ...
                <p className="text-slate-400 max-w-2xl">Dile a la IA qué perfil de clientes estás buscando hoy. El motor analizará la petición y optimizará el targeting para la campaña de Patchright automáticamante.</p>
              </div>

              <MagicBox onStrategyApplied={(data) => {
                ...
                setCampaignDraft({
                  name: buildCampaignDisplayName('', activeAccount?.ig_username, data.sources),
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
                  <div className="flex items-start justify-between gap-4 mb-5">
                    <div>
                      ...
                      <p className="text-sm text-slate-400 mt-1">Combina hashtags, followers y location. La IA te propone fuentes compuestas para explorar primero y luego explotar lo que mejor convierta.</p>
                    </div>
                    <Badge variant="secondary" className="bg-slate-800 text-slate-300">
                      {campaignDraft.sources.length} source{campaignDraft.sources.length === 1 ? '' : 's'}
                    </Badge>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-[1.2fr_0.8fr_0.8fr]">
                    <div className="space-y-2 md:col-span-3">
                      ...
                      <input
                        ...
                        onChange={(e) => setCampaignDraft((prev) => ({ ...prev, name: e.target.value.slice(0, 80) }))}
                        placeholder="Ej: Constructoras CABA"
                    ...
                    <div className="space-y-2 md:col-span-2">
                      ...
                      <div className="rounded-xl border border-slate-800 bg-slate-950/60 px-4 py-3 text-slate-200">
                        {activeAccount ? `@${activeAccount.ig_username}` : 'Sin cuenta conectada'}
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <GlowSelect
                        value={campaignDraft.executionMode}
                        onChange={(next) => setCampaignDraft((prev) => ({ ...prev, executionMode: next as CampaignDraft['executionMode'] }))}
                        options={[
                  ...
                  <p className="mt-3 text-xs text-slate-500">
                    Real usa tu sesion autenticada y el extractor verdadero para hashtag y followers. Test solo simula el pipeline sin tocar Instagram ni el CRM.
                  </p>
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          onChange={(e) => setCampaignDraft((prev) => ({ ...prev, minFollowers: Number(e.target.value) || 0 }))}
                          className="w-full rounded-xl border border-slate-800 bg-slate-950/60 px-4 py-3 text-slate-200 outline-none focus:border-purple-500"
                        />
                      </div>
                      <div className="space-y-2">
                        ...
                        <input
                          type="number"
                          min={0}
                          value={campaignDraft.minPosts}
                          onChange={(e) => setCampaignDraft((prev) => ({ ...prev, minPosts: Number(e.target.value) || 0 }))}
                          className="w-full rounded-xl border border-slate-800 bg-slate-950/60 px-4 py-3 text-slate-200 outline-none focus:border-purple-500"
                        />
                      </div>
                      <div className={`rounded-2xl border px-4 py-3 transition-colors md:col-span-2 ${campaignDraft.requireCoherence ? 'border-emerald-500/40 bg-emerald-500/10' : 'border-slate-800 bg-slate-950/60'}`}>
                        <div className="flex items-center justify-between gap-4">
                          ...
                          <button
                            type="button"
                            role="switch"
                            aria-checked={campaignDraft.requireCoherence}
                            onClick={() => setCampaignDraft((prev) => ({ ...prev, requireCoherence: !prev.requireCoherence }))}
                            className={`relative inline-flex h-9 w-16 shrink-0 items-center rounded-full border transition-colors ${campaignDraft.requireCoherence ? 'border-emerald-400/70 bg-emerald-500/30' : 'border-slate-700 bg-slate-800'}`}
                          >
                  ...
                  <div className="mt-4 space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <label className="text-xs font-semibold uppercase tracking-wider text-slate-500">Sources de targeting</label>
                      <div className="flex flex-wrap gap-2">
                        <button onClick={() => addSource('hashtag')} className="rounded-lg bg-slate-800 px-3 py-1.5 text-xs font-medium text-slate-200 hover:bg-slate-700">+ Hashtag</button>
                        <button onClick={() => addSource('followers')} className="rounded-lg bg-slate-800 px-3 py-1.5 text-xs font-medium text-slate-200 hover:bg-slate-700">+ Followers</button>
                        <button onClick={() => addSource('location')} className="rounded-lg bg-slate-800 px-3 py-1.5 text-xs font-medium text-slate-200 hover:bg-slate-700">+ Location</button>
                      </div>
                    </div>
                    ...
                    {campaignDraft.sources.map((source, index) => {
                      return (
                        <div key={`${source.type}-${index}`} className="grid grid-cols-1 md:grid-cols-[180px_1fr_auto] gap-3 rounded-xl border border-slate-800 bg-slate-950/60 p-3">
                          <GlowSelect
                            value={source.type}
                            onChange={(next) => setSourceAt(index, 'type', next)}
                            options={[
                          ...
                          <div className="flex items-center rounded-xl border border-slate-800 bg-slate-900 px-4 py-3">
                            <span className="mr-2 text-slate-500">{source.type === 'hashtag' ? '#' : source.type === 'followers' ? '@' : ''}</span>
                            <input
                              type="text"
                              value={source.target}
                              onChange={(e) => setSourceAt(index, 'target', e.target.value)}
                              placeholder={source.type === 'hashtag' ? 'Ej: brokersinmobiliarios' : source.type === 'followers' ? 'Ej: constructora' : 'Ej: miamicondos'}
                              className="w-full bg-transparent text-slate-100 outline-none placeholder:text-slate-600"
                            />
                          </div>
                          <button
                            onClick={() => removeSource(index)}
                            disabled={campaignDraft.sources.length === 1}
                            className="rounded-xl border border-slate-800 px-4 py-3 text-sm font-medium text-rose-300 hover:bg-rose-500/10 disabled:cursor-not-allowed disabled:opacity-40"
                          >
                            Quitar
                          </button>
                        </div>
                  ...
                  <div className="mt-5 flex flex-wrap gap-3">
                    <button
                      onClick={async () => {
                        ...
                        const cleanedSources = campaignDraft.sources
                          .map((source) => ({ ...source, target: source.target.trim() }))
                        ...
                        try {
                          const res = await fetch('http://localhost:8000/api/bot/start', {
                            ...
                            body: JSON.stringify({
                              ...
                              sources: cleanedSources.map((source) => ({
                                type: source.type,
                                value: source.target,
                              })),
                          ...
                          await mutateBotStatus();
                          toast.success(`Campana preparada con ${cleanedSources.length} source(s)`, {
                            description: `Cuenta emisora: @${activeAccount.ig_username}.`
                          });
                    ...
                    <button
                      onClick={() => {
                        setCampaignDraft({ name: '', sources: [{ type: 'followers', target: '' }], limit: 5, executionMode: 'real', minFollowers: 50, minPosts: 3, requireCoherence: true });
                  ...
                  {lastStrategy ? (
                    <div className="mt-4 space-y-4">
                      <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-4">
                        <p className="text-xs uppercase tracking-wider text-slate-500 mb-2">Objetivo seleccionado</p>
                        <div className="flex flex-wrap gap-2">
                          {lastStrategy.sources.map((source, index) => (
                            <Badge key={`${source.type}-${source.target}-${index}`} variant="outline" className="border-slate-700 text-slate-300">
                              {source.type === 'hashtag' ? '#' : source.type === 'followers' ? '@' : ''}{source.target}
                            </Badge>
                          ))}
                        </div>
                      </div>
                      <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-4">
                        <p className="text-xs uppercase tracking-wider text-slate-500 mb-2">Razonamiento</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl relative overflow-hidden group hover:border-purple-500/30 transition-colors">
                  ...
                  <div className="mt-4 flex items-center text-xs text-purple-400 font-medium bg-purple-500/10 w-fit px-2 py-1 rounded-full">
                    Sincronizado via SQLite
                  </div>
                </div>

                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl relative overflow-hidden group hover:border-amber-500/30 transition-colors">
                  <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                    <Activity className="w-16 h-16 text-amber-400" />
                  </div>
                  <p className="text-sm font-medium text-slate-400 mb-1">En Cola (Pendientes)</p>
                  <p className="text-4xl font-bold text-slate-100">{pendientes}</p>
                  <div className="mt-4 flex items-center text-xs text-amber-400 font-medium bg-amber-500/10 w-fit px-2 py-1 rounded-full">
                    Esperando Outreach
                  </div>
                </div>

                <div className="bg-slate-900 border border-emerald-500/30 rounded-2xl p-6 shadow-xl relative overflow-hidden group">
                  <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                    <BadgeCheck className="w-16 h-16 text-emerald-400" />
                  </div>
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="text-sm font-medium text-slate-400 mb-1">Trust Score (IA)</p>
                      <p className="text-4xl font-bold text-emerald-400">{trustScore}%</p>
                    </div>
                    <Badge className="bg-emerald-500/10 text-emerald-400 border-none">Seguro</Badge>
                  </div>
                  <div className="mt-4 w-full bg-slate-800 rounded-full h-1.5">
                    <div className="bg-emerald-500 h-1.5 rounded-full" style={{ width: `${trustScore}%` }}></div>
                  </div>
                </div>
              </div>
                        ...
                        <tr key={i} className="bg-slate-900 border-b border-slate-800/50 hover:bg-slate-800/50 transition-colors">
                          <td className="px-6 py-4">
                            ...
                            {lead.last_message_preview && (
                              <div className="mt-2 max-w-sm rounded-lg border border-cyan-500/20 bg-cyan-500/5 px-2 py-1 text-xs text-cyan-100">
                                {lead.last_message_preview.length > 140 ? `${lead.last_message_preview.slice(0, 140)}...` : lead.last_message_preview}
                              </div>
                            )}
                            {lead.last_message_rationale && <div className="mt-2 text-xs text-slate-500">{lead.last_message_rationale}</div>}
                          </td>
                          <td className="px-6 py-4">
                            <Badge variant="outline" className={getLeadStatusTone(lead.status)}>
                              {lead.status}
                            </Badge>
                            {lead.last_outreach_result && (
                              <div className="mt-2 text-xs text-slate-500">Resultado: {lead.last_outreach_result}</div>
                            )}
                            {lead.last_outreach_error && (
                              <div className="mt-2 max-w-xs text-xs text-rose-300">{lead.last_outreach_error}</div>
                            )}
                          </td>
                          <td className="px-6 py-4 text-slate-400">
                            <span className="rounded-full border border-slate-700 bg-slate-950/60 px-3 py-1 text-xs text-slate-300">{formatSourceLabel(lead.source)}</span>
                          </td>
                        </tr>
                      )) : (
                        <tr>
                          <td colSpan={3} className="px-6 py-8 text-center text-slate-500">
                            Cargando data del CRM o sin resultados...
                          </td>
                        </tr>
                      )}
                    </tbody>
                      ...
                      <tr>
                        <th className="w-12 bg-slate-900 px-3 py-2">
                          <input
                            ...
                            onChange={(e) => e.target.checked ? selectVisibleLeads() : clearLeadSelection()}
                          ...
                          <td className="px-3 py-2 align-middle text-right">
                            <div className="flex items-center justify-end">
                              <div className="relative group">
                                ...
                                <div className="invisible absolute right-0 top-full z-20 mt-1 flex w-44 flex-col overflow-hidden rounded-lg border border-slate-700 bg-slate-800 p-1 opacity-0 shadow-xl transition-all group-hover:visible group-hover:opacity-100">
                                  <button
                                    onClick={() => window.open(`https://www.instagram.com/${lead.username}/`, '_blank', 'noopener,noreferrer')}
                                    className="w-full rounded-md px-3 py-2 text-left text-xs font-medium text-slate-300 transition-colors hover:bg-slate-700 hover:text-white"
                                  >
                                    🕵️ Ver Perfil
                                  </button>
                                  <button
                                  ...
                                  {accountNeedsRelogin && (
                                    <button
                                      onClick={reloginActiveAccount}
                                      disabled={isReloggingAccount}
                                      className="w-full rounded-md px-3 py-2 text-left text-xs font-medium text-rose-300 transition-colors hover:bg-rose-500/20 disabled:opacity-50"
                                    >
                                      {isReloggingAccount ? '🔐 Re-logueando...' : '🔐 Re-loguear cuenta'}
                                    </button>
                                  )}
                                  {lead.status.startsWith('Error') && typeof lead.id === 'number' && (
                                    <button
                                      ...
                                      className="w-full rounded-md px-3 py-2 text-left text-xs font-medium text-amber-300 transition-colors hover:bg-amber-500/20"
                                    >
                                      ↺ Reactivar
                                    </button>
                                  )}
                                  {typeof lead.id === 'number' && (
                                    <button
                                      onClick={async () => {
                                        try {
                                          const res = await fetch(`http://localhost:8000/api/leads/${lead.id}`, { method: 'DELETE' });
                                          if (!res.ok) {
                                            const data = await res.json();
                                      ...
                                      className="mt-1 w-full rounded-md border-t border-slate-700/50 px-3 py-2 text-left text-xs font-medium text-rose-400 transition-colors hover:bg-rose-500/20 hover:text-rose-300"
                                    >
                                      🗑️ Eliminar
                                    </button>
                                  )}
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 space-y-6">
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-xl">
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2">
                  <button onClick={previewPendingDraftsFromMessages} disabled={isPreparingDrafts || isSavingDrafts || messageScopeLeadIds.length === 0} className="rounded-lg bg-slate-800 px-3 py-1.5 text-xs font-medium text-slate-100 hover:bg-slate-700 disabled:opacity-40">
                    {isPreparingDrafts ? 'Generando...' : 'Generar muestra IA'}
                  </button>
                  <button onClick={updatePendingDraftsFromMessages} disabled={isPreparingDrafts || isSavingDrafts || messageScopeLeadIds.length === 0} className="rounded-lg bg-white px-3 py-1.5 text-xs font-semibold text-slate-950 hover:bg-slate-200 disabled:opacity-40">
          ...
          {currentView === 'guide' && (
            <div className="animate-in fade-in slide-in-from-bottom-4 space-y-6">
              <div>
                <h2 className="text-3xl font-bold tracking-tight text-slate-100">Cómo usarlo</h2>
                <p className="mt-2 text-slate-400">Guía operativa completa para usar Botardium de forma segura, clara y predecible.</p>
              </div>

              <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
                <h3 className="text-lg font-semibold text-slate-100">Flujo recomendado (end-to-end)</h3>
                <ol className="mt-4 list-decimal space-y-2 pl-5 text-sm text-slate-300">
                  <li>Conecta una cuenta emisora y define su tipo: madura, nueva o rehabilitación.</li>
                  <li>Si la cuenta requiere calentamiento de cuenta, completa días de warmup antes de outreach.</li>
                  <li>Desde Dashboard crea campaña(s) para scraping y alimenta el CRM.</li>
                  <li>En CRM mueve leads listos a <span className="text-slate-100">Listo para contactar</span>.</li>
                  <li>En Message Studio ajusta Prompt Maestro + prompts por etapa.</li>
                  <li>Actualiza borradores masivos según estado/campaña.</li>
                  <li>En CRM selecciona leads válidos, prepara cuenta emisora si hace falta y envía.</li>
                  <li>Repite follow-ups manualmente, cuidando límites diarios y señales de riesgo.</li>
                </ol>
              </div>

              <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
                <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
                  <h3 className="text-lg font-semibold text-slate-100">Qué hace cada vista</h3>
                  <div className="mt-4 space-y-3 text-sm text-slate-300">
                    <p><span className="font-medium text-slate-100">Dashboard:</span> arma y lanza campañas de scraping.</p>
                    <p><span className="font-medium text-slate-100">CRM de Leads:</span> estados, selección, envío y operaciones rápidas.</p>
                    <p><span className="font-medium text-slate-100">Message Studio:</span> Prompt Maestro + prompts por etapa y actualización masiva de borradores.</p>
                    <p><span className="font-medium text-slate-100">Campañas:</span> seguimiento de pipelines, logs y source stats.</p>
                    <p><span className="font-medium text-slate-100">Cuentas:</span> salud de cuenta emisora, warmup de cuenta y relogin.</p>
                  </div>
                </div>
                <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
                  <h3 className="text-lg font-semibold text-slate-100">Estados CRM y envío</h3>
                  <div className="mt-4 space-y-3 text-sm text-slate-300">
                    <p><span className="font-medium text-slate-100">Pendiente:</span> aún no se debe enviar.</p>
                    <p><span className="font-medium text-slate-100">Listo para contactar:</span> habilitado para primer envío.</p>
                    <p><span className="font-medium text-slate-100">Primer contacto / Follow-up 1:</span> habilitados para siguiente paso.</p>
                    <p><span className="font-medium text-slate-100">Follow-up 2:</span> último toque; después queda <span className="text-slate-100">Completado</span>.</p>
                    <p><span className="font-medium text-slate-100">Completado, Respondio, Calificado, No interesado:</span> estados cerrados (no envío masivo).</p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
                <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
                  <h3 className="text-lg font-semibold text-slate-100">Prompt Maestro (Message Studio)</h3>
                  <div className="mt-4 space-y-3 text-sm text-slate-300">
                    <p><span className="font-medium text-slate-100">Prompt inicial:</span> usa configuración base recomendada.</p>
                    <p><span className="font-medium text-slate-100">Prompt personalizado:</span> ajusta tono/estilo global para toda la secuencia.</p>
                    <p>Los prompts de <span className="text-slate-100">Primer contacto</span>, <span className="text-slate-100">Follow-up 1</span> y <span className="text-slate-100">Follow-up 2</span> refinan cada etapa.</p>
                    <p>Las reglas duras de seguridad/calidad quedan fijas en backend y no son editables desde UI.</p>
                  </div>
                </div>
                <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
                  <h3 className="text-lg font-semibold text-slate-100">Calentamientos: diferencia crítica</h3>
                  <div className="mt-4 space-y-3 text-sm text-slate-300">
                    <p><span className="font-medium text-slate-100">Calentamiento de cuenta:</span> proceso de varios días para cuentas nuevas/rehab.</p>
                    <p><span className="font-medium text-slate-100">Preparar cuenta emisora (sesión):</span> warmup corto antes de enviar bloque de DMs.</p>
                    <p>Preparar sesión no &quot;calienta leads&quot;; prepara la cuenta para entregar mejor el siguiente lote.</p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
                ...
                <div className="mt-4 space-y-3 text-sm text-slate-300">
                  <p><span className="font-medium text-slate-100">¿Los follow-ups se envían solos por fecha?</span> No. Hoy son manuales para mantener control operativo.</p>
                  <p><span className="font-medium text-slate-100">¿Reiniciar backend borra fechas o estados?</span> No, quedan persistidos en base de datos local.</p>
                  <p><span className="font-medium text-slate-100">¿Por qué no me deja enviar?</span> Revisa selección: no incluir Pendiente ni estados cerrados.</p>
                  <p><span className="font-medium text-slate-100">¿Qué hago si warmup se corta?</span> Usa re-login de cuenta y vuelve a preparar cuenta emisora.</p>
                </div>
              </div>
            </div>
          )}
